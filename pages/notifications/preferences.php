...
$eventTypes = getNotificationEventTypes();
...
$categories = [
    ...
    'marketing' => ['label' => 'Marketing',           'icon' => 'bi-tag'],
    'security'  => ['label' => 'Security',            'icon' => 'bi-shield-exclamation'],
    'system'    => ['label' => 'System',              'icon' => 'bi-gear'],
];

// Exclude supplier-only events from the user preferences page
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrf() && isset($_POST['reset_defaults'])) {
    ...
    header('Location: preferences.php?saved=2');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrf() && isset($_POST['save_prefs'])) {
    $newPrefs = [];
    foreach ($eventTypes as $type => $meta) {
        if (!in_array($meta['category'], $userCategories, true)) {
            continue;
        }
        $newPrefs[$type] = [
            'in_app' => 1, // always on
...
<div class="container py-4">
    ...
    <form method="POST">
        ...
        <?php foreach ($categories as $catKey => $catInfo): ?>
            <?php
            $catEvents = array_filter($eventTypes, static fn($m) => $m['category'] === $catKey);
